...
public class JoystickInputController {
   ...
   */
  public JoystickInputController(
      Drive drive,
  ...
  public Optional<Rotation2d> getHeadingDirection() {
    ...
    return JoystickInputController.getHeadingDirection(
        xAngleSupplier.getAsDouble(), yAngleSupplier.getAsDouble());
  }

  /**
   * Get if the controller is getting input
   *
   * @return if the controller is getting input
   */
  @AutoLogOutput(key = "Drive/TeleopDriveController/isGettingInput")
  public boolean isGettingInput() {
    return JoystickUtil.applyDeadband(xSupplier.getAsDouble()) != 0
        || JoystickUtil.applyDeadband(ySupplier.getAsDouble()) != 0
        || JoystickUtil.applyDeadband(xAngleSupplier.getAsDouble()) != 0
        || JoystickUtil.applyDeadband(yAngleSupplier.getAsDouble()) != 0;
  }
